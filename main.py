...
def main(page: ft.Page):
    ...
    # --- 1. REFRESH DASHBOARD (Must be first) ---
    ...
    def show_checkout_dialog(qr_id):
        ...
        page.dialog = ft.AlertDialog(
            title=ft.Text(f"Check-Out Machine"),
            content=worker_name,
    ...
    def handle_scanned_tool(qr_id):
        ...
        if not tool:
            ...
            # Direct to checkout to save time
            show_checkout_dialog(qr_id)
        else:
            # Automatic return if already checked out
            db.update_tool_status(qr_id, "Warehouse", "Available", None)
            refresh_dashboard()
        page.update()

    # --- 5. CORE SCANNER & UPLOAD ---
    def on_scan_result(e: ft.FilePickerResultEvent):
        if e.files:
            ...
            for f in e.files:
                # IMPORTANT: We use the local file path provided by Flet's picker
                # On mobile, this connects to the 'Take Photo' result
                img = cv2.imread(f.path if f.path else f.name)
                ...
                if img is not None:
                    ...
                    if data:
                        ...
                        page.snack_bar = ft.SnackBar(
                            ft.Text("QR not found. Try a clearer photo."),
                            bgcolor=ft.Colors.RED,
                        )
            page.update()

    # --- 6. PDF REPORTS (Web Launch Fix) ---
    def create_pdf(e):
        report_type = e.control.text.split()[0]
        filename = reports.generate_report(report_type)
        # Using page.launch_url is the only way to open PDF on Web
...
if __name__ == "__main__":
    ...
    # 'static_dir' must be set to "." so the browser can see your PDF files
    ft.app(target=main, view=None, port=port, static_dir=".")
