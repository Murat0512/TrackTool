...
def main(page: ft.Page):
    ...
    # CHANGED: ThemeMode instead of theme_mode
    ...
    def handle_scanned_tool(qr_id):
        ...
        if not tool:
            ...
            # CHANGED: Colors.GREEN instead of colors.green
            page.snack_bar = ft.SnackBar(
                ft.Text(f"Tool {qr_id} returned successfully!"), bgcolor=ft.Colors.GREEN
            )
            page.snack_bar.open = True
            refresh_dashboard()
    ...
    def show_checkout_dialog(qr_id):
        ...
        # CHANGED: Icons.PERSON instead of icons.person
        ...
        def confirm_checkout(e):
            ...
            # CHANGED: Colors.BLUE instead of colors.blue
            page.snack_bar = ft.SnackBar(
                ft.Text(f"Assigned to {worker_name.value}"), bgcolor=ft.Colors.BLUE
            )
            page.snack_bar.open = True
            refresh_dashboard()
            page.update()
    ...
    page.add(
        # CHANGED: Colors.AMBER_700 instead of colors.AMBER_700
        ft.AppBar(title=ft.Text("TrackTool Pro"), bgcolor=ft.Colors.AMBER_700),
        ft.Text("Active Inventory", size=20, weight="bold"),
        tool_list,
        ft.FloatingActionButton(
            # CHANGED: Icons.QR_CODE_SCANNER instead of icons.QR_CODE_SCANNER
            icon=ft.Icons.QR_CODE_SCANNER,
            text="Scan Tool",
            on_click=open_scanner,
            # CHANGED: Colors.AMBER_ACCENT_400 instead of colors.AMBER_ACCENT_400
